...
// Builds everything from source, including the CLI and its dependencies.
//
// The shipping `Package.swift` declares the CLI as a prebuilt binary instead, so a consumer
// that only wants `GoldenMaster` resolves nothing. This manifest is what CI, contributors,
// and `scripts/build-artifactbundle.py` build from.
